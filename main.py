# main.py - WattScope LIGHT (garanti fonctionnel)
...
async def ajouter_releve(foyer_id: int = Form(...), date_releve: str = Form(...), index_compteur: float = Form(...), duree_coupure: float = Form(0), unite_coupure: str = Form("minutes"), temperature: str = Form(""), cout_estime: float = Form(0.0), db: Session = Depends(get_db)):
...
async def dashboard(request: Request, foyer_id: int, db: Session = Depends(get_db), success: str = ""):
    ...
    app_html = ""; app_labels = "[]"; app_data = "[]"; top_app = ""
    if apps:
    ...
    for r in releves[:20]:
        ...
        co=f"{r.cout_estime_fcfa} FCFA" if r.cout_estime_fcfa else "N/A"
        ...
        rel_html+=f"<tr><td>{r.date_releve}</td><td><strong>{r.index_compteur}</strong></td><td>{coup}</td><td>{t}</td><td>{co}</td></tr>"
...
async def export_excel(foyer_id: int, db: Session = Depends(get_db)):
    ...
    for i,h in enumerate(["Date","kWh","Coupure(min)","Temp(°C)","Coût(FCFA)"],1):
        c=ws.cell(row=1,column=i,value=h);c.font=hf;c.fill=hfl
    for i,r in enumerate(list(client.releves),2):
        ws.cell(row=i,column=1,value=str(r.date_releve))
        ws.cell(row=i,column=2,value=r.index_compteur)
        ws.cell(row=i,column=3,value=r.duree_coupure_minutes)
        ws.cell(row=i,column=4,value=r.temperature_exterieure or 0)
        ws.cell(row=i,column=5,value=r.cout_estime_fcfa or 0)
...
async def ajouter_appareil(foyer_id: int = Form(...), nom_appareil: str = Form(...), puissance_watts: int = Form(...), heures: float = Form(1.0), db: Session = Depends(get_db)):
